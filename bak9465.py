...
1 초	256 MB	32947	16681	10919	48.918%
문제
...
T = int(stdin.readline()) # test 케이스
n = int(stdin.readline()) # 열

stk = [list(map(int,stdin.readline().split())) for _ in range(2)] # 스티커

dp = [0 for _ in range(n)]

'''
https://pacific-ocean.tistory.com/197
이 규칙을 적용했을때의 표는 아래와 같다.
...
여기에서 마지막 인덱스 중 큰값을 출력해주면 된다.
'''
stk[0][1] += stk[1][0]
stk[1][1] += stk[0][0]
for j in range(2,n):
    stk[0][j] += max(stk[1][j-1],stk[1][j-2])
    stk[1][j] += max(stk[0][j-1],stk[0][j-2])
